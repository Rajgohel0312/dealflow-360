...
import { invalidateCache, withCache } from "../../shared/utils/cacheHelper.js";

export const convertQuotationToOrder = async (quotationId, salesRepId, userRole) => {
  const lockKey = `dealflow:lock:order:${quotationId}`;
  const lock = await CacheService.acquireLock(lockKey, 10000);

  if (!lock.acquired) {
    throw new AppError("Order conversion is already in progress for this quotation. Please wait.", 409);
  }
  ...
  try {
    ...
    await invalidateCache("dealflow:dashboard:*");
    ...
    await CacheService.releaseLock(lockKey, lock.lockValue);
  }
};
